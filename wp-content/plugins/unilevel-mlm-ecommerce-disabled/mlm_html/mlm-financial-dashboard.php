...
$user_name = $wpdb->get_var("SELECT username FROM ".MLM_USERS." where user_id='".$user_id."'");

 $mlm_currency = get_option('wp_mlm_general_settings');
 $currency=MLM_CURRENCY;       
 
$balance = $wpdb->get_var("SELECT closing_bal FROM ".MLM_TRANSACTION."  WHERE id = (select max(id) from ".MLM_TRANSACTION." where user_id='".$user_id."')");
$withdrwl_bal=$wpdb->get_var("SELECT SUM(round(amount+withdrawal_fee+witholding_tax)) as total FROM ".MLM_WITHDRAWAL."  where payment_processed=0 AND user_id='".$user_id."'");
...
<style>.payout-summary tr { margin:10px 0px; }</style>
	<!--<script src="initiate.js" type="text/javascript"></script>-->
        ...
        <span style='color:green;'><?php  echo    !empty($sucsess)?$sucsess:''; ?></span>
		<form name="form" action="" method="post" >
        <table width="100%" border="0" cellspacing="10" cellpadding="1" class="payout-summary">
		<tr><td><strong>User Name:</strong>&nbsp;&nbsp;&nbsp;<strong><?php  echo   $user_name ?></strong></td>
        <td><strong>Account Balance:</strong>&nbsp;&nbsp;&nbsp;<strong><?php  echo   $currency ?><?php  echo   empty($balance)?'0.00':$balance ?></strong></td></tr>
  
<tr><td>&nbsp;</td>
        <td><strong>Withdrawable Balance:</strong>&nbsp;&nbsp;&nbsp;<strong><?php  echo   $currency ?><?php  echo   empty($Left_balance)?'0.00':$Left_balance ?></strong></td></tr>
  
				<tr>
                <td><strong>Withdrawal Amount</strong></td>
                <td width="50%"><input type="text" name="wamount" id="wamount"  onblur="return numeric(this.value,'wamount');" value="<?php  echo  empty($wamount)?'':$wamount ?>"  size="21" /></td>
                </tr>
                <td><strong>Comment </strong></td>
				<td width="50%" ><textarea name="wcomment" id="wcomment"><?php  echo  empty($wcomment)?'':$wcomment ?></textarea></td>
                </tr>
                <tr>
                ...
                <option value='<?php  echo   $values ?>' <?php if(isset($_POST['withdrawalMode'])){ if($deduction['withdwl_mthd'][$i]==$minAmt[0]){ echo 'selected';}  }?>><?php  echo   $deduction['withdwl_mthd'][$i] ?></option>
                <?php } ?>
                </select></td></tr>
...
<?php 
       ...
       $result = $wpdb->get_results("select * from ".MLM_TRANSACTION." where user_id={$user_id} order by transaction_date ASC");
      $records = array_reverse($result);
       $totalrecords = count($records); 
	   
	  //$othr_method = $wpdb->get_var("select other_method from ".MLM_WITHDRAWAL." where user_id ='1' AND id=21");
...
function drawVisualization() {
  ...
  var dataAsJson =
  {cols:[
	{id:'A',label:'<?php  echo   _e("Date",MLM_PLUGIN_NAME); ?>',type:'string'},
	{id:'B',label:'<?php  echo   _e("Opening Bal",MLM_PLUGIN_NAME); ?>',type:'string'},
	{id:'C',label:'<?php  echo   _e("Dr. Amount",MLM_PLUGIN_NAME); ?>',type:'string'},
        {id:'C',label:'<?php  echo   _e("Cr.Amount",MLM_PLUGIN_NAME); ?>',type:'string'},
        {id:'C',label:'<?php  echo   _e("Closing Bal",MLM_PLUGIN_NAME); ?>',type:'string'},
	{id:'D',label:'<?php  echo   _e("Comment",MLM_PLUGIN_NAME); ?>',type:'string'}],
  rows:[
   <?php foreach( $records as $row) : 
       ...
       if($row->dr_id!='0'){
           ...
           $withdrawal_fee = $wpdb->get_var("select withdrawal_fee from ".MLM_WITHDRAWAL." where user_id ='{$row->user_id}' AND id='{$row->dr_id}'");
           $witholding_tax = $wpdb->get_var("select witholding_tax from ".MLM_WITHDRAWAL." where user_id ='{$row->user_id}' AND id='{$row->dr_id}'");
		   $othrs_method='';
		   $othrs_method = $wpdb->get_var("select other_method from ".MLM_WITHDRAWAL." where user_id ='{$row->user_id}' AND id='{$row->dr_id}'");
...
$rowcount++;
	 ?>
      {c:[{v:'<?php  echo   DefaultDateFormat($row->transaction_date) ?>'},
                        {v:'<?php  echo   number_format($wt_opening, 2, '.', '') ?>'},
                        {v:'<?php  echo   number_format($othr_method[1][$i], 2, '.', '') ?>'},
                        {v:'<?php  echo   number_format($row->cr_amount, 2, '.', '') ?>'},
                        {v:'<?php  echo   number_format($closing, 2, '.', '') ?>'},
                        {v:'<?php  echo   'Other Deductions: '.$othr_method[0][$i]?>'}]},
<?php
}
	...
	<?php 
	 ...
	 ?>
	{c:[{v:'<?php  echo   DefaultDateFormat($row->transaction_date) ?>'},
                        {v:'<?php  echo  number_format($row->closing_bal+$wf+$wt, 2, '.', '') ?>'},
                        {v:'<?php  echo  number_format($wf, 2, '.', '') ?>'},
                        {v:'<?php  echo  number_format($row->cr_amount, 2, '.', '') ?>'},
                        {v:'<?php  echo   number_format($row->closing_bal+$wt, 2, '.', '') ?>'},
                        {v:'<?php  echo  $wf_comment ?>'}]},
	<?php } ?>		
		{c:[{v:'<?php  echo   DefaultDateFormat($row->transaction_date) ?>'},
                        {v:'<?php  echo  number_format($row->opening_bal, 2, '.', '')?>'},
                        {v:'<?php  echo  number_format($row->dr_amount-$wf-$wt, 2, '.', '') ?>'},
                        {v:'<?php  echo  number_format($row->cr_amount, 2, '.', '') ?>'},
                        {v:'<?php  echo   number_format($row->closing_bal+$wf+$wt, 2, '.', '') ?>'},
                        {v:'<?php  echo  $row->comment?>'}]},
...
<!--va-matter-->
    <div class="va-matter">
    	...
    	<div class="va-matterbox">
        	<!--va-headname-->
        	<div class="va-headname"><?php _e('My Transaction History (All figures below are in '.$currency.')',MLM_PLUGIN_NAME);?></div>
            <!--/va-headname-->
			<div class="va-admin-leg-details">
				...
				<div class="paging">
				  <form action="">
					...
					<?php _e('Total Records',MLM_PLUGIN_NAME);?>: <strong><?php  echo   $rowcount; ?></strong>
					</div>
