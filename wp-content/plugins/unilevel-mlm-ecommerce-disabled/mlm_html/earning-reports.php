<div class="wrap">
			...
			<h2><?php _e('Earning Reports',MLM_PLUGIN_NAME);?></h2>
</div>
<?php
	...
	$currency = MLM_CURRENCY;
	 ...
	 extract($_REQUEST);
	  ...
	  $firstuser_id = $wpdb->get_var("SELECT user_id FROM ".MLM_USERS." WHERE username = '".$firstUser."'");
	  
	  
	$total_paid_users=$wpdb->get_var("SELECT count(*) as total from ".MLM_USERS." as mu INNER JOIN {$table_prefix}users as wu ON mu.user_id = wu.ID WHERE mu.user_id!='".$firstuser_id."' AND payment_status='1' $between");
	
	$firstu_id = $wpdb->get_var("SELECT id FROM ".MLM_USERS." WHERE username = '".$firstUser."'");
	$payout_paid=$wpdb->get_var("SELECT sum(total_amt) as amount from ".MLM_PAYOUT." WHERE user_id!='".$firstu_id."' $between1");
	
	if(isset($total_paid_users))
	{
	$total_paid_users=$total_paid_users;
	//$total_amount=$total_paid_users*$product_price;
	$total_amount=$wpdb->get_var("SELECT sum(product_price) as total from ".MLM_USERS." WHERE payment_status='1' AND payment_date BETWEEN '$datefromfinal' AND '$datetofinal'");
	}
	else
	{
		$total_paid_users='0';
...
<form id="processed_report" method="GET" action="">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
                <tr>
                <td align='left'><strong><?php _e('Date From',MLM_PLUGIN_NAME);?></strong><strong>:</strong></td>
                <td align='right'><input type="text" name="datefrom" id="datefrom"></td>
                </tr>
                <tr><td><br/></td></tr>
                <tr>
                <td align='left'><strong><?php _e('Date To',MLM_PLUGIN_NAME);?></strong><strong>:</strong></td>
...
<table cellspacing="0" class="wp-list-table widefat fixed toplevel_page_admin-mlm-reports">
    ...
    <tr><td><strong><?php _e('Gross Earnings :',MLM_PLUGIN_NAME);?></strong>&nbsp;&nbsp;<?php echo $currency." ".$total_amount;?><br/><br/></td></tr>
<tr><td><strong><?php _e('Payouts :',MLM_PLUGIN_NAME);?></strong>&nbsp;&nbsp;<?php echo $currency." ".$payout_paid;?><br/><br/></td></tr>
<tr><td><strong><?php _e('Net Earnings :',MLM_PLUGIN_NAME);?></strong>&nbsp;&nbsp;<?php echo $currency." ".$net_earning;?><br/><br/></td></tr>
