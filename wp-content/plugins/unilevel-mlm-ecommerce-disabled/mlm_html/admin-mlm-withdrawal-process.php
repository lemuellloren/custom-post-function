...
?>
<div class='wrap'>
	<div id="icon-users" class="icon32"></div><h1><?php _e('Process Individual User Withdrawal',MLM_PLUGIN_NAME);?></h1><br />
	<div class="notibar msginfo" style="margin:10px;">
		<a class="close"></a>
		<p><?php _e('Use the form below to process an individual user withdrawal.',MLM_PLUGIN_NAME);?></p>
		<p><strong><?php _e('Cash',MLM_PLUGIN_NAME);?></strong> - <?php _e('Simply records a cash payment against the withdrawal with no further details.',MLM_PLUGIN_NAME);?></p>
		<p><strong><?php _e('Cheque',MLM_PLUGIN_NAME);?></strong> - <?php _e('Specify the Cheque Number, Cheque Date and Bank Name.',MLM_PLUGIN_NAME);?></p>
		<p><strong><?php _e('Bank Transfer',MLM_PLUGIN_NAME);?></strong> - <?php _e('Specify the Beneficiary Name, Account Number, Bank Name and Bank Transfer Code (optional).',MLM_PLUGIN_NAME);?></p>
		<p><strong><?php _e('Other',MLM_PLUGIN_NAME);?></strong> - <?php _e('For any other mode of payment. Just specify the payment details in the input box provided.',MLM_PLUGIN_NAME);?></p>
	</div>	
</div>
...
<?php	
	...
	$sql = "UPDATE ".MLM_WITHDRAWAL." SET `cheque_no`='".$cheque_no."',
	...
	$res= $wpdb->query($sql);
	$closing_bal = $wpdb->get_var("SELECT closing_bal FROM ".MLM_TRANSACTION."  WHERE id = (select max(id) from ".MLM_TRANSACTION."  where user_id='".$user_id."')");
	$opening_bal = $closing_bal;
        $closing_bal = $opening_bal - $amount;
	$wpdb->query("INSERT INTO ".MLM_TRANSACTION."   set 
                                user_id ='".$user_id."', 
                                dr_id = '".$id."',
                  ...
                  WithDrawalMemberMail($user_id,$withdrawalMode,$wamount);                    
                  }
                  if($res=='1') {  
...
<form method="POST" action="" id="paydone_form" name="payment_complete">
	<table border="0" cellpadding="5" cellspacing="0">
		...
		<tr>
			<td><?php _e('Details/Comments',MLM_PLUGIN_NAME)?></td>
			<td>
		...
		<tr>
			...
			<td>
				...
				<input type="submit" name="paydone" id="paydone" value="<?php _e('Process',MLM_PLUGIN_NAME)?>">
			</td>
		</tr>
	</table>
</form>
