...
/**
 * Core Class for Unilevel MLM Ecommerce
...
if (!class_exists('UMP')) {
    ...
    class UMP {
        ...
        function UMP() { // constructor
            ...
            $this->PluginOptionName = 'UMP_Options';
            $this->TablePrefix = $wpdb->prefix . 'ump_';
...
//			$this->pluginURL = plugins_url('', $this->pluginPath);
        ...
        function Plugin_Info_Hook($res, $action, $args) { 
            if ($res === false && $action == 'plugin_information' && $args->slug == 'uni-level-mlm-pro') {
                $res = new stdClass();
                ...
                $res->requires = "3.0";
                $res->sections = array(
                    'description' => '<p>Unilevel MLM Ecommerce is a powerful solution for creating an online membership site â€“ all built using WordPress as the core content management system.</p>
                        <p>Now itâ€™s easy to control access to your content, accept payments, manage your members and so much more! Read below for full feature descriptions, tutorial videos and examples of sites using Unilevel MLM Pro.</p>',
                    'support' => '<p>Need help?  Click one of the links below.</p>
            <ul>
            <li><a href='.WP_BINARY_MLM_ULR.'"/faqs" target="_blank">FAQs</a></li>
            <li><a href='.WP_BINARY_MLM_ULR.'"/product/priority-support" target="_blank">Priority Support</a></li>
            <li><a href='.WP_BINARY_MLM_ULR.'"/blog" target="_blank">Blog</a></li>
            </ul>'
                );
        ...
        function Pre_Upgrade($return, $plugin) {
            ...
            if ($plugin == $this->PluginFile) {
                $dir = sys_get_temp_dir() . '/' . 'UMP-Upgrade';
        ...
        function UpdateNag() {
            if (!$this->Plugin_Is_Latest()) {
                $latest_ump_ver = $this->Plugin_Latest_Version();
                if (!$latest_ump_ver) {
                    $latest_ump_ver = $this->Version;
                }

                global $current_user;
                $user_id = $current_user->ID;
                $dismiss_meta = 'dismiss_mlm_update_notice_' . $latest_ump_ver;
                if (!get_user_meta($user_id, $dismiss_meta) && current_user_can('update_plugins')) {
                    echo "<div class='update-nag'>";
                    printf(__("The most current version of Unilevel MLM Ecommerce is v%s.", 'wishlist-member'), $latest_ump_ver);
                    echo " ";
        ...
        function Plugin_Latest_Version() {
            static $latest_ump_ver;
            $varname = 'myplugin_version';
            if (empty($latest_ump_ver) OR isset($_GET['checkversion'])) {
                //   $latest_ump_ver = get_transient($varname);
                if (empty($latest_ump_ver) OR isset($_GET['checkversion'])) {
                    $latest_ump_ver = plugin_Latest_Version();
                    if (empty($latest_ump_ver)) {
                    ...
                    set_transient($varname, $latest_ump_ver, 60 * 60 * 24);
                }
            }
            return $latest_ump_ver;
        }
        ...
        function Plugin_Latest_Version_key() {
            ...
            if (empty($latest_ump_ver) OR isset($_GET['checkversion'])) {
                //   $latest_ump_ver = get_transient($varname);
                if (empty($latest_ump_ver) OR isset($_GET['checkversion'])) {
                    $latest_ump_ver = plugin_Latest_Version_key();
                    if (empty($latest_ump_ver)) {
                        //we failed, set the latest version to this one so that we won't keep checking again for today
                        $latest_ump_ver = $this->Version;
                    }
