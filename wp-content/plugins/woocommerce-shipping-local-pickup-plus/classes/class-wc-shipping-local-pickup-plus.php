...
class WC_Shipping_Local_Pickup_Plus extends WC_Shipping_Method {
	...
	public function __construct() {
		...
		$this->method_title = __( 'Local Pickup Plus', WC_Local_Pickup_Plus::TEXT_DOMAIN );

		$this->admin_page_description  = __( 'Local pickup is a simple method which allows the customer to pick up their order themselves at a specified pickup location.', WC_Local_Pickup_Plus::TEXT_DOMAIN );
	...
	public function calculate_shipping( $package ) {
		...
		if ( $location ) {
			...
			if ( $cost && $range[0] != $range[1] ) {
				$label .= ' ' . __( 'from', WC_Local_Pickup_Plus::TEXT_DOMAIN );
			}
		}

		// if there's a discount for local pickup, add a note to the displayed shipping method title
		if ( $this->get_discount() ) {
			$label .= ' (' . sprintf( __( 'save %s', WC_Local_Pickup_Plus::TEXT_DOMAIN ), strip_tags( wc_price( $this->get_discount() ) ) ) . ')';
		}
	...
	public function is_available( $package ) {
		...
		if ( "no" == $this->enabled || 0 == count( $this->pickup_locations ) ) return false;
	...
	public function init_form_fields() {
		...
		$categories = array( 0 => __( 'All Categories', WC_Local_Pickup_Plus::TEXT_DOMAIN ) );
		...
		$this->form_fields = array(
			...
			'enabled' => array(
				'title'   => __( 'Enable', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable local pickup plus', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default' => 'no',
			),

			'title' => array(
				'title'       => __( 'Title', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'        => 'text',
				'description' => __( 'This controls the title which the user sees during checkout.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default'     => __( 'Local Pickup', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
			),

			'cost' => array(
				'title'       => __( 'Cost', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'        => 'text',
				'description' => __( 'Default cost excluding tax. Enter an amount, e.g. 2.50, or leave empty for no default cost.  The default cost can be overriden by setting a cost per pickup location below.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default'     => '',
			),

			'discount' => array(
				'title'       => __( 'Discount', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'        => 'text',
				'description' => __( 'Discount for choosing Local Pickup as the shipping option.  Enter an amount, e.g. 2.50 to discount the order total (post-tax) when this shipping method is used.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default'     => '',
			),

			'categories' => array(
				'title'       => __( 'Categories', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'        => 'multiselect',
				'description' => __( 'Optional, these categories of products may only be locally picked up.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'options'     => $categories,
				'class'       => 'chosen_select',
			),

			'categories_pickup_only' => array(
				'title'   => __( 'Categories Only', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'    => 'checkbox',
				'label'   => __( 'Allow local pickup only for the product categories listed above, all other product categories must be shipped via another method', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default' => 'no',
			),

			'apply_pickup_location_tax' => array(
				'title'   => __( 'Pickup Location Tax', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'    => 'checkbox',
				'label'   => __( 'When this shipping method is chosen, apply the tax rate based on the pickup location than for the customer\'s given address.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default' => 'no',
			),

			'hide_shipping_address' => array(
				'title'   => __( 'Hide Shipping Address', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'    => 'checkbox',
				'label'   => __( 'Hide the shipping address when local pickup plus is selected at checkout.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'default' => 'no',
			),

			'checkout_pickup_location_styling' => array(
				'title'   => __( 'Pickup Location Styling', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
				'type'    => 'select',
				'options' => array( 'select' => __( 'Dropdown', WC_Local_Pickup_Plus::TEXT_DOMAIN ), 'radio' => __( 'Radio Buttons', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ),
				'default' => 'select',
				'desc_tip' => __( 'Styling of pickup location options on checkout.', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
			),
	...
	public function admin_options() {
		...
		<table class="form-table">
			...
			<tr valign="top">
				<th scope="row" class="titledesc"><?php _e( 'Pickup Locations', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?>:</th>
				<td class="forminp" id="<?php echo $this->id; ?>_pickup_locations">
					<table class="shippingrows widefat" cellspacing="0">
						<thead>
							<tr>
								...
								?>
								<th><?php _e( 'Cost', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?> <img class="help_tip" width="15" height="15" style="float:none;" data-tip='<?php _e( 'Cost for this pickup location, enter an amount, eg. 0 or 2.50, or leave empty to use the default cost configured above.', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" /></th>
								<th><?php _e( 'Notes', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?> <img class="help_tip" width="15" height="15" style="float:none;" data-tip='<?php _e( 'Free-form notes to be displayed below the pickup location on checkout/receipt.  HTML content is allowed.', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ?>' src="<?php echo WC()->plugin_url(); ?>/assets/images/help.png" /></th>
							</tr>
						</thead>
						<tfoot>
							<tr>
								<th colspan="<?php echo count( $pickup_fields ) + 3 ?>"><a href="#" class="add button"><?php _e( '+ Add Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?></a>
								<a href="#" class="remove button"><?php _e( 'Delete Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?></a></th>
							</tr>
						</tfoot>
						...
						if ( $this->pickup_locations ) foreach ( $this->pickup_locations as $location ) {
							...
							foreach ( $pickup_fields as $key => $field ) {
								...
								if ( 'state' == $key ) {
									...
									// all other fields
									echo '<input type="text" name="' . $this->id . '_' . $key . '[]" value="' . $location[ $key ] . '" placeholder="' . ( in_array( $key, array( 'company', 'address_2', 'phone' ) ) ? __( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) : '' ) . '" />';
								}

								echo '</td>';
							}
							echo '<td><input type="text" name="' . $this->id . '_cost[]" value="' . ( isset( $location['cost'] ) ? $location['cost'] : '' ) . '" placeholder="' . __( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '" /></td>';
							echo '<td><textarea name="' . $this->id . '_note[]" placeholder="' . __( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '">' . ( isset( $location['note'] ) ? $location['note'] : '' ) . '</textarea></td>';
							echo '</tr>';
						}
						?>
						</tbody>
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
							  ...
							  <td class="check-column" style="width:20px;"><input type="checkbox" name="select" />\
								...
								foreach ( $pickup_fields as $key => $field ) {
									...
									if ( 'state' == $key ) {
										...
										echo '<input type="text" name="' . $this->id . '_' . $key . '[]" value="" placeholder="' . ( in_array( $key, array( 'company', 'address_2', 'phone' ) ) ? __( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) : '' ) . '" />';
									}

									echo '</td>';
								}
							?><td><input type="text" name="<?php echo $this->id ?>_cost[]" value="" placeholder="<?php _e( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ?>" /></td>\
							  <td><textarea name="<?php echo $this->id ?>_note[]" placeholder="<?php _e( '(Optional)', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ?>"></textarea></td>\
				...
				$( '#<?php echo $this->id; ?>_pickup_locations a.remove' ).live( 'click', function() {
					var answer = confirm( "<?php _e( 'Delete the selected pickup locations?', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?>" );
					if ( answer ) {
	...
	public function review_order_shipping_pickup_location( $template_name, $template_path, $located, $args ) {
		...
		if ( 'cart/cart-shipping.php' == $template_name &&
			$this->id == $args['chosen_method'] && (
				is_checkout() ||
				( defined( 'DOING_AJAX' ) && isset( $_REQUEST['action'] ) && 'woocommerce_update_order_review' == $_REQUEST['action'] )
			) ) {
			...
			echo '<tr class="pickup_location">';
			echo '<th colspan="1">' . ( 1 == count( $this->pickup_locations ) ? __( 'Your Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN  ) : __( 'Choose Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . ' <abbr class="required" title="required" style="border:none;">*</abbr>' ) . '</th>';
			...
			if ( 1 == count( $this->pickup_locations ) ) {
				...
				if ( 'select' == $this->get_checkout_pickup_location_styling() ) {
					echo '<select name="pickup_location[' . $args['index'] . ']" class="pickup_location">';
					echo '<option value="">' . __( 'Choose One', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '</option>';
					foreach ( $this->pickup_locations as $location ) {
					...
					// radio styling (TODO requires WC 2.1.x?)
	...
	public function review_order_pickup_location() {
		...
		if ( in_array( $this->id, $this->get_chosen_shipping_methods() ) ) {
			...
			echo '<tr class="pickup_location">';
			echo '<th colspan="1">' . ( 1 == count( $this->pickup_locations ) ? __( 'Your Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN  ) : __( 'Choose Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ) ) . '</th>';
			echo '<td class="update_totals_on_change">';
			...
			if ( 1 == count( $this->pickup_locations ) ) {
				...
				echo '<select name="pickup_location[0]" class="pickup_location">';
				echo '<option value="">' . __( 'Choose One', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '</option>';
				foreach ( $this->pickup_locations as $location ) {
			...
			foreach ( $shipping_method as $package_index => $shipping_method_id ) {
				...
				if ( $this->id == $shipping_method_id ) {
					...
					if ( ! isset( $pickup_location[ $package_index ] ) || ! is_numeric( $pickup_location[ $package_index ] ) ) {
						wc_add_notice( __( 'Please select a local pickup location', WC_Local_Pickup_Plus::TEXT_DOMAIN ), 'error' );
						return;
					}
				}
			}
	...
	public function woocommerce_check_cart_items() {
			...
			if ( isset( $shipping_packages[ $package_id ]['contents'] ) ) {
				...
				if ( $this->id == $shipping_method_id ) {
					...
					if ( 'yes' == $this->categories_pickup_only && count( $other_products ) > 0 ) {
						wc_add_notice( sprintf( __( 'Some of your cart products are not eligible for local pickup, please remove %s, or select a different shipping method to continue', WC_Local_Pickup_Plus::TEXT_DOMAIN ), '<strong>' . implode( ', ', $other_products ) . '</strong>' ), 'error' );
						$has_errors = true;
					}
				} else {
					// some other shipping method selected
					if ( count( $found_products ) > 0 && ! $allow_other_methods_for_found_categories ) {
						wc_add_notice( sprintf( __( 'Some of your cart products are only eligible for local pickup, please remove %s, or change the shipping method to %s to continue', WC_Local_Pickup_Plus::TEXT_DOMAIN ), '<strong>' . implode( ', ', $found_products ) . '</strong>', $this->title ), 'error' );
						$has_errors = true;
					}
				}
			}
		...
		if ( $has_errors && is_cart() ) {
			...
			wc_enqueue_js( 'jQuery( "body" ).bind( "updated_shipping_method", function() { location.reload(); } );' );
		}
	}


	/**
	 * If there is a discount configured for local pickup plus and this is
	 * the chosen shipping method, apply the discount
	 *
	 * @since 1.4.4
	...
	public function apply_discount( $cart ) {
		if ( $this->get_discount() ) {
			$cart->discount_total += $this->get_discount();
		}
	}
	...
	public function cart_shipping_method_full_label( $full_label, $method ) {
		...
		$index = strlen( ' (' . __( 'Free', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . ')' );

		if ( $this->get_discount() && substr( $full_label, -$index ) == ' (' . __( 'Free', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . ')' ) {
		...
		if ( isset( $_POST['pickup_location'][ $package_index ] ) &&
			...
			if ( isset( $shipping_methods[ $shipping_item_id ]['method_id'] ) && $this->id == $shipping_methods[ $shipping_item_id ]['method_id'] ) {
				...
				foreach ( $packages[ $package_index ]['contents'] as $cart_item_id => $values ) {
					...
					wc_update_order_item_meta( $order_item_id, __( 'Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ), $this->get_formatted_address_helper( $pickup_location, true ) );
	...
	public function email_pickup_location( $order ) {
		...
		if ( count( $pickup_locations ) > 0 ) {
			echo '<div>';
			echo '<h3>' . _n( 'Pickup Location', 'Pickup Locations', count( $pickup_locations ), WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '</h3>';
		}
	...
	public function email_pickup_location_plain( $order ) {
		...
		if ( count( $pickup_locations ) > 0 ) {
			echo _n( 'Pickup Location', 'Pickup Locations', count( $pickup_locations ), WC_Local_Pickup_Plus::TEXT_DOMAIN );
		}
		...
		if ( $order->has_shipping_method( $this->id ) ) {
				...
				jQuery( document ).ready( function( $ ) {
					...
					$( '.edit_pickup_location select' ).change( function() {
						...
						$.each( orderLineItems, function( index, lineItemId ) {
							...
							var meta_key = $( '.item[data-order_item_id="' + lineItemId + '"] input[type="text"][value="<?php _e( 'Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?>"]' );
							...
							if ( meta_key.length > 0 ) {
								...
								$( '.item[data-order_item_id="' + lineItemId + '"] .display_meta tr' ).each( function( index, element ) {
									if ( '<?php _e( 'Pickup Location', WC_Local_Pickup_Plus::TEXT_DOMAIN ); ?>:' == $( element ).find( 'th' ).text() ) {
			...
			foreach ( $shipping_methods as $shipping_item_id => $shipping_item ) {
				...
				if ( $this->id == $shipping_item['method_id'] ) {
					...
					if ( ! $title_shown ) {
						echo '<h4>' . __( 'Pickup Locations', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '<a class="edit_pickup_location" href="#"><img src="' . WC()->plugin_url() . '/assets/images/icons/edit.png" alt="Edit" width="14" /></a></h4>';
						$title_shown = true;
					}
					...
					if ( $formatted_pickup_location ) {
						...
						echo '<p class="none_set">' . __( 'No pickup location set.', WC_Local_Pickup_Plus::TEXT_DOMAIN ) . '</p>';
					}
		...
		foreach ( $pickup_locations as $shipping_item_id => $pickup_location_id ) {
			...
			if ( wc_update_order_item_meta( $shipping_item_id, 'pickup_location', $pickup_location ) ) {
				...
				$order->add_order_note(
					sprintf( __( 'Pickup location changed to %s', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
	...
	public static function get_chosen_shipping_methods() {
		return WC()->session->get( 'chosen_shipping_methods' ) ? WC()->session->get( 'chosen_shipping_methods' ) : array();
	}
	...
	public function get_chosen_pickup_location_id( $package_index ) {
		...
		$pickup_location_ids = WC()->session->get( 'chosen_pickup_locations' );
	...
	private function get_pickup_address_fields( $country ) {
		...
		foreach ( $address_fields as $key => $value ) {
			...
			if ( 'address_2' == $key ) {
				$pickup_fields[ $key ]['label'] = __( 'Address 2', WC_Local_Pickup_Plus::TEXT_DOMAIN );
			}
		}
		...
		if ( ! isset( $pickup_fields['phone'] ) ) {
			$pickup_fields['phone'] = array(
				'label' => __( 'Phone', WC_Local_Pickup_Plus::TEXT_DOMAIN ),
			);
		}
